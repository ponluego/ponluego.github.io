...
function oobstaculo() {
  ...
  obstaculo = new THREE.Mesh(geometry, material);
  obstaculo.castShadow = true;
  obstaculo.position.x = 10;
  scene.add(obstaculo);
}
oobstaculo();
...
// GUI
{
  ...
  const guip = gui.addFolder("panel FV");
  guip.add(gi, "latitud", 0, 90).step(1);
  guip.add(gi, "inclinacion", 0, 90).step(1);
  guip.add(gi, "orientacion", -180, 180).step(1);
  const guio = gui.addFolder("obstaculo");
  guio.add(gi, "altura", 0, 2.5).step(.1);
  guio.add(gi, "sur", 0, 20).step(1);
  guio.add(gi, "oeste", -20, 20).step(1);
  const guis = gui.addFolder("sol");
  guis.add(gi, "mes", 0, 12).step(1);
  guis.add(gi, "hora", 0, 24).step(1);
...
function bucle() {
  ...
  obstaculo.scale.y=gi.altura
  obstaculo.position.x=gi.sur
  obstaculo.position.z=gi.oeste
